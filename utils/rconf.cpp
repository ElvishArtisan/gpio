...
//
//    $Id: rconf.cpp,v 1.2 2011/09/16 21:20:26 cvs Exp $
//
...
bool GetLock(const char *sLockname)
{
  ...
  if((fd=open(sLockname,O_WRONLY|O_EXCL|O_CREAT,S_IRUSR|S_IWUSR))<0) {
    printf("failed!\n");
    if(RCheckPid(RGetPathPart(sLockname),RGetBasePart(sLockname))) {
      return false;
    }
    ClearLock(sLockname);
    if((fd=open(sLockname,O_WRONLY|O_EXCL|O_CREAT,S_IRUSR|S_IWUSR))<0) {
...
QString RGetShortDate(QDate date)
{
...
QFont::Weight RGetFontWeight(QString string)
{
...
QString RGetDisplay(bool strip_point)
{
#ifdef WIN32
  ...
  if(getenv("DISPLAY")[0]==':') {
    display=QString().sprintf("%s%s",(const char *)RGetHostAddr().toString(),
			     getenv("DISPLAY"));
  }
  else {
...
bool RDoesRowExist(QString table,QString name,QString test,QSqlDatabase *db)
{
  QSqlQuery *q;
  QString sql;
...
bool RDoesRowExist(QString table,QString name,unsigned test,QSqlDatabase *db)
{
  QSqlQuery *q;
  QString sql;
...
QString RGetTimeLength(int mseconds,bool leadzero,bool tenths)
{
...
int RSetTimeLength(QString str)
{
  int istate=2;
  QString field;
  int res=0;
...
#ifndef WIN32
bool RWritePid(QString dirname,QString filename,int owner,int group)
{
  FILE *file;
  mode_t prev_mask;
...
bool RCheckPid(QString dirname,QString filename)
{
  ...
  QString path;
  path=QString("/proc/")+
    QString().sprintf("%d",RGetPid(dirname+QString("/")+filename));
  dir.setPath(path);
  return dir.exists();
}
...
QString RGetHomeDir(bool *found)
{
  if(getenv("HOME")==NULL) {
    if(found!=NULL) {
      *found=false;
...
QString RTruncateAfterWord(QString str,int word,bool add_dots)
{
