...
//
//    $Id: rgpio.cpp,v 1.1 2007/11/07 20:36:32 fredg Exp $
//
...
RGpio::RGpio(QObject *parent,const char *name)
  : QObject(parent,name)
{
  Clear();
...
QString RGpio::device() const
{
  return gpio_device;
}


void RGpio::setDevice(QString dev)
{
  gpio_device=dev;
}


QString RGpio::description() const
{
  return gpio_description;
}


RGpio::Mode RGpio::mode()
{
...
void RGpio::setMode(RGpio::Mode mode)
{
...
bool RGpio::open()
{
  ...
  if(ioctl(gpio_fd,GPIO_GETINFO,&gpio_info)==0) {
    gpio_api=RGpio::ApiGpio;
    InitGpio();
    RemapTimers();
  }
  else {
    if(ioctl(gpio_fd,EVIOCGVERSION,&ver)==0) {
      gpio_api=RGpio::ApiInput;
      InitInput();
    }
    else {
      ::close(gpio_fd);
...
bool RGpio::inputState(int line)
{
  if(!gpio_open) {
    return false;
  }
...
unsigned RGpio::outputMask() const
{
  struct gpio_mask mask;
...
void RGpio::gpoSet(int line,unsigned interval)
{
...
void RGpio::gpoReset(int line,unsigned interval)
{
  struct gpio_line out;
...
void RGpio::SetReversion(int line,unsigned interval)
{
  if(interval==0) {
